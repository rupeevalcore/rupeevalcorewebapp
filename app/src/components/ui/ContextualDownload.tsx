...
export function ContextualDownload({ title, file, brochureKey, audience, className }: ContextualDownloadProps) {
  const downloadUrl = brochureKey ? (BROCHURES[brochureKey]?.pdfPath || "#") : (file ? `/downloads/${file}` : "#");
  const fileName = downloadUrl.split("/").pop() || "brochure.pdf";
  
  const handleDownloadClick = () => {
    trackEvent("file_download", {
      file_name: brochureKey || file || fileName,
      link_text: title,
      section: audience,
    });
  };
    ...
    <div className={cn("flex flex-col", className)}>
      <script type="application/ld+json">
        {JSON.stringify({
          ...
          "@type": "DataDownload",
          "name": title,
          "contentUrl": `https://www.rupeevalcore.in${downloadUrl}`,
